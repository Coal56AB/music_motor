...
int main(void)
{
  ...
  /* Establish safe driver levels before waiting for the external oscillator. */
  platform_safe_gpio();
  /* USER CODE END Init */
...
static void MX_USART1_UART_Init(void)
{
  ...
  /* Config is authoritative for baud; CubeMX keeps the peripheral setup. */
  if (huart1.Init.BaudRate != UART_BAUD) {
